...
<ol class="breadcrumb">
  ...
  <li><a href="<?= $this->Url->build(['controller'=>'Items', 'action'=>'viewReptiles']) ?>">カテゴリ（爬虫類）</a></li> 
</ol>
<div class="row">
        <!-- left -->
        <div class="col-md-3">
            ...
            <div class="panel panel-default">
                ...
                <ul class="nav nav-pills nav-stacked">
                    ...
                    <li><a href="<?= $this->Url->build(['controller'=>'Items', 'action'=>'viewReptile']) ?>"><div class="check check-active"></div><span class="op-name">爬虫類</span></a></li>
        ...
        <div class="col-md-9">
            <div class="col-md-6">
                ...
                <option value="?sort=created&direction=asc"
                <?php
                    if(strstr($url, 'sort=created&direction=asc')){
                        echo "selected";
                    }
                 ?>>古い順</option>
                <option value="?sort=created&direction=desc"
                <?php
                    if(strstr($url, 'sort=created&direction=desc')){
                        echo "selected";
                    }
                ?>>新着順</option>
                <option value="?sort=item_price&direction=asc"
                <?php
                    if(strstr($url, 'sort=item_price&direction=asc')){
                        echo "selected";
                    }
                ?>>価格が安い順</option>
                <option value="?sort=item_price&direction=desc"
                <?php
                    if(strstr($url, 'sort=item_price&direction=desc')){
                        echo "selected";
                    }
                ?>>価格が高い順</option>
                <option value="?sort=modified&direction=desc"
                <?php
